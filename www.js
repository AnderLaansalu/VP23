...
http.createServer(function(req, res){
	...
	/////Ülalolev probleemne(nüüd korras)!!!! Kui collect ära kustutada, on ok. Muidu appendis paneks nime andmed. NB! Süntaksile erilist tähelepanu pidada.
			   ...
			   if (err) {
				 ...
				 listAllNames(res, htmlOutput);
					...
					for (person of allData){
						extractedData.push(person.split(','));
					}
					for (person of extractedData) {
						if (person[0]) {
						htmlOutput += '\n\t\t<li>' + person [0] + ' ' + person[1] + ', kuupäev: ' + person[2] + '</li>';
					}
				}
				htmlOutput += '\n\t</ul>'
				listAllNames(res, htmlOutput);
		...
		//	Keerukam versioon üleval, tagastab konsoolile massiivid, allolev lehele listina. Kodus puhas enda versiooni backup samuti
			// else if (currentURL.pathname === "/addNameValues") {
			// 	fs.readFile('public/log.txt', 'utf8', (err, data) => {
			// 		if (err) {
			// 	 		throw err;
			...
			// 	});
			// }
		//Ülemise ja selle kommentaari vahel mid-versioon, kommenteeri välja kui vaja
